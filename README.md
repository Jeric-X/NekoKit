...
| `cateye_search` | 核心 | 以图搜图（华为云 / trace.moe / SauceNAO） |
| `cateye_vision` | 核心 | 大模型图片理解（日常模式 / 专业模式） |
| `cateye_preprocess` | 辅助 | 按任务类型优化图片尺寸和格式 |
| `cateye_cache` | 辅助 | 检查/存储缓存，避免重复调用 |
| `cateye_scene` | 编排 | 场景预设，返回工具组合策略 |

核心工具专注于识别逻辑，不内置缓存和预处理。辅助工具由智能体根据场景预设方案按需调用。`cateye_scene` 提供内置场景预设，智能体也可自行修改预设方案实现任务自适应。
...
| [Cateye 使用指南](docs/agent_guides/cateye.md) | AI 智能体 | 工具 Schema、调用示例、场景预设 |
| [KV 存储设计文档](docs/design/kv_store.md) | 开发者 | 设计理念、命名空间策略、扩展性 |
| [Cateye 设计文档](docs/design/cateye.md) | 开发者 | 工具解耦、场景预设、供应商抽象 |
